from backend.models import Operation, Job, Action
from backend.factory_logic_loader import FactoryLogic
from backend.machine_runtime import MachineRuntime
...
class Factory():
    ...
    def apply_actions(self, actions: Dict[str, Optional[Action]]) -> None:
        ...
        for machine_id, action in actions.items():
            if action is not None:
                ...
                print(f"DEBUG: Dispatched operation: {machine_id, action.job_id, action.operation_id, action.task_mode_id}")
            else:
                print(f"DEBUG: No action for machine {machine_id}")
    ...
    def step(self) -> float:
        ...
        total_power_cost = self.get_total_power_cost(power_consumed, self.current_step)
    ...
    def get_total_power_cost(self, power_consumed: float, step: int) -> float:
        """
